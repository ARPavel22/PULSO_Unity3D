...
public class PULSO_InteractiveObject : MonoBehaviour
{
    ...
    public void Vibrate(float freq, float amp, float time, PULSO_HandpadNew.HandSide handSide)// OVRInput.Controller handSide)
    {
        StartCoroutine(Vibrate_process(freq, amp, time, handSide));
    }

    IEnumerator Vibrate_process(float freq, float amp, float time, PULSO_HandpadNew.HandSide handSide)// OVRInput.Controller handSide)
    {
